 ...
 *
 *   $Id: include.mod_subnav.php 358 2008-06-27 12:53:37Z frederic.schneider $:
 * }}
 * 
 */
...
if ( isset($_GET['idmod']) && $idmod != 0 ) {
    ...
    while ( $db->next_record() ) {
        ...
        if ($perm->have_perm_area_action($tmp_area))
        {
            ...
            $tpl->set("d", "CLASS",     '');
            $tpl->set("d", "OPTIONS",   '');
            $tpl->set("d", "CAPTION",   '<a class="white" onclick="sub.clicked(this)" target="right_bottom" href="'.$sess->url("main.php?area=$tmp_area&frame=4&idmod=$idmod").'">'.$caption.'</a>');
            $tpl->next();
